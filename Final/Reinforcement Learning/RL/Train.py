...
WEIGHTS_PATH = WORK_FOLDER + r'\weights'
...
Decision_me = Action_Choose(objective=OBJECTIVE_MY, threshold=0.25)
Decision_opponent = Action_Choose(objective=OBJECTIVE_OPPONENTS, threshold=0.25)

critic_network = CriticNetwork(parameters=[7 * 4 + 2, 16, 1])  # 神经网络结构
...
for i in range(5000):
    ...
    while not env.is_end():
        ...
        if env.role == ME:
            moves, values = get_move(ME)
            move, value = Decision_me.forward(moves, values)
            if env.is_end() and env.is_end()==1:
                reward = 1.0
            else:
                reward = 0.0
            err = critic_network.back_propagation(env, move, role=ME, reward=reward)
        else:
            moves, values = get_move(OPPONENT)
            move, value = Decision_opponent.forward(moves, values)
            err = critic_network.back_propagation(env, move, role=OPPONENT, reward=0)
        if i % 100 == 0:
            critic_network.LR = 0.8*critic_network.LR
    end_t = time.time()
    print('Game %s set. Winner: ' % i + ('ME' if env.is_end() == 1 else 'OPPONENT'))
    print('train time:', end_t-start_t)
    win_record.append('ME' if env.is_end() == 1 else 'OPPONENT')
    if env.is_end() == 1:
        win_me += 1
