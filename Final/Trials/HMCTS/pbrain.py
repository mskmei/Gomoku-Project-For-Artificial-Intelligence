...
def brain_turn():
    ...
    state = mcts.State(cboard, 1, my, oppo)
    root = mcts.Node(None, None, state)
    root.actions = root.state.get_node_actions()  # 通过局部heuristic确定合理的落子位置，并优先应对威胁
    if len(root.actions) == 1:
        x, y = root.actions[0]
    else:
        x, y = root.best_action()
    pp.do_mymove(x, y)
