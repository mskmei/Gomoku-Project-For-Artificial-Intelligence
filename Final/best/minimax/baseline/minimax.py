...
def brain_turn():
    ...
    a = threat_search(board, pp.width, pp.height)
    if a is not None:
        x, y = a
    else:
        x, y = search(board, pp.width, pp.height)
    pp.do_mymove(x, y)
    return
    ...
    def five_judge(self, x, y, role):
        ...
        for dir in direction_list:
            ...
            for mukau in range(len(dir)):
                ...
                j = y + dir[mukau][1]
                while True:
                    if i >= self.width or j >= self.height or i < 0 or j < 0 or self.board[i][j] == 3-role:
                        break
                    else:
                        count += 1
                        i += dir[mukau][0]
...
def my_threat(board, role, alpha, beta, depth, m_depth):
    global hash
    if depth >= m_depth:
        if hash in zobristHash:
            utility = zobristHash[hash]
    ...
    if len(action_list) != 0:  # 有地方可下
        ...
        for a in action_list:
            ...
            board.place[role - 1].append((a[1], a[2]))
            hash ^= ranTable[a[1]][a[2]][role - 1]
            move_v, _ = oppo_threat(board, role, alpha, beta, depth + 1, m_depth)
            board.board[a[1]][a[2]] = 0
            board.place[role - 1].remove((a[1], a[2]))
            hash ^= ranTable[a[1]][a[2]][role - 1]
            if move_v >= w*DOUBLE_THREE:
                return move_v, a  # 检测到必胜情况，直接返回
            if move_v > v:
                v = move_v
                action = a
...
def oppo_threat(board, role, alpha, beta, depth, m_depth):
    global hash
    if depth >= m_depth:
        if hash in zobristHash:
            utility = zobristHash[hash]
    ...
    if len(action_list) != 0:
        ...
        for a in action_list:
            ...
            board.place[2 - role].append((a[1], a[2]))
            hash ^= ranTable[a[1]][a[2]][2 - role]
            move_v, _ = my_threat(board, role, alpha, beta, depth + 1, m_depth)
            board.board[a[1]][a[2]] = 0
            board.place[2 - role].remove((a[1], a[2]))
            hash ^= ranTable[a[1]][a[2]][2 - role]
...
def threat_search(current_board, width, height):
    P = Board(current_board, width, height)
    alpha = float("-inf")
    beta = float("inf")
    role = 1
    ...
    for d in range(1, max_threat_depth + 1):
        depth = 0
        _, action = my_threat(P, role, alpha, beta, depth, d)
        if action:
            break
    if action:
        return action[1], action[2]
